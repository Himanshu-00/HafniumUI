...
def ensure_image_consistency(image, target_size=(1024, 1024), aspect_ratio='center_crop'):
    """
    Standardize image dimensions while preserving aspect ratio and content.
    
    Args:
        image (PIL.Image): Input image
        target_size (tuple): Desired output size
        aspect_ratio (str): Cropping method ('center_crop' or 'resize_pad')
...
def segment_and_refine_mask(image, target_size=(1024, 1024)):
    """
    Enhanced segmentation with consistent image processing.
    """
    # Ensure consistent image size and aspect ratio
    consistent_image = ensure_image_consistency(image, target_size)
    
    # Rest of your existing segmentation logic remains the same
    segmented = remove(consistent_image)
    mask = Image.new("L", consistent_image.size, 0)
    
    # Convert transparency into grayscale mask
    for y in range(segmented.height):
        for x in range(segmented.width):
            _, _, _, a = segmented.getpixel((x, y))
            mask.putpixel((x, y), a)
    
    # Subsequent processing steps...
    return mask, consistent_image
