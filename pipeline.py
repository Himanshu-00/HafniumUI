...
# pipeline.py (modified for progressive image streaming)
import os
import torch
...
def generate_image_with_lora(pipeline, prompt, negative_prompt, guidance_scale, num_steps, input_image, num_images=1):
    try:
        ...
        print(f"Generating {num_images} images with guidance scale: {guidance_scale}, steps: {num_steps}")
        ...
        # Generate mask once for consistency
        mask = segment_and_refine_mask(input_image)
        generated_images = []

        # Generate and yield images progressively
        for _ in range(num_images):
            with torch.no_grad():
                result = pipeline(
                    prompt=PROMPT,
            ...
            generated_images.append(image)  # Store in list
