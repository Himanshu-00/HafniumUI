...
# gradio_interface.py
import gradio as gr
from pipeline import generate_images, pipeline_with_lora
from config import NPROMPT
...
def create_gradio_interface():
    ...
    with gr.Blocks(theme=theme) as HafniumUI: 
        ...
        # Row with two columns
        with gr.Row():
            # Left side column with input image (with mask annotation), color selection, guidance scale, and steps
            with gr.Column():
                # Use gr.ImageMask to let users both upload an image and annotate a mask on it.
                # If no mask is drawn (or the drawn mask is empty), the default segmentation method is used.
                input_image = gr.ImageMask(
                    label="Input Image (annotate mask if desired)", 
                    type="pil"
                )
                
                # Radio buttons for suit colors, with a default value.
                color_picker = gr.Radio(
                    choices=[
                        "Charcoal (#3b3b3b)", "Black (#000000)", "Navy Blue (#000080)", 
                        "Gray (#808080)", "White (#FFFFFF)", "Dark Brown (#654321)", 
                ...
                # Row for guidance scale and steps sliders
                with gr.Row():
            ...
            # Right side column for output images
            with gr.Column(min_width=800):
                output_image = gr.Gallery(
                    label="Generated Images",
                    elem_id="output_gallery",
            ...
            # Button click: pass the inputs (including the annotated mask)
            generate_btn.click(
                fn=generate_images,
                inputs=[color_picker, guidance_scale, steps, input_image, num_outputs],
                outputs=output_image,
