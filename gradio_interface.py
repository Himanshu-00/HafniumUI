...
def create_gradio_interface(pipeline_with_lora):
    with gr.Blocks() as HafniumUI: 
        ...
        with gr.Row():
            # Left side column with input_image, color selection, guidance_scale, and steps
            with gr.Column():
                input_image = gr.Image(label="Input Image", type="pil", tool="sketch")  # Add brush tool for masking
            ...
            # Button functionality with queue
            generate_btn.click(
                fn=generate_images,
                inputs=[color_picker, guidance_scale, steps, input_image, num_outputs],
                outputs=output_image,
                queue=True  # Enable queue for processing
            )
