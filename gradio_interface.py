...
def create_gradio_interface(pipeline_with_lora):
    with gr.Blocks(theme=gr.themes.Citrus()) as HafniumUI:  # Apply citrus theme
        gr.Markdown("# SDXL with LoRA Integration and Inpainting")  # Just the markdown without color argument
        ...
        with gr.Row():
            ...
            # Update button appearance dynamically with a background color change
            generate_btn.update(
                background_color="#FFEB3B", text_color="black", hover_background_color="#FFC107"
            )
