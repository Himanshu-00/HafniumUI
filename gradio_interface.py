...
def create_gradio_interface(pipeline_with_lora):
    ...
    with gr.Blocks(theme=theme) as HafniumUI:
        ...
        with gr.Row():
            with gr.Column():
                # Add masking mode selector
                mask_mode = gr.Radio(
                    choices=["Automatic Masking", "Manual Masking"],
                    label="Masking Mode",
                    value="Automatic Masking",
                    interactive=True
                )
                
                # Input image with mask drawing capability
                input_image = gr.Image(
                    label="Input Image",
                    tool="sketch",  # Enable drawing tools
                    type="pil",
                    source="upload",
                )
            ...
            # Show/hide mask drawing instructions based on mode
            mask_mode.change(
                fn=lambda x: gr.update(tool="sketch" if x == "Manual Masking" else None),
                inputs=[mask_mode],
                outputs=[input_image]
            )

            # Update generate button click to include mask mode
            generate_btn.click(
                fn=generate_images,
                inputs=[color_picker, guidance_scale, steps, input_image, num_outputs, mask_mode],
                outputs=output_image,
                show_progress=True
            )
